...
describe.skipIf(!existsSync(`${DIR}/Test0.pdf`))('batch', () => {
  it('all 10 books', async () => {
    ...
    for (let i = 0; i <= 9; i++) {
      ...
      console.log(`## Test${i}: ${pages.length}p | cit=${res.citations.length} rows=${rows.length} (main=${main.length} rev=${review.length} lit=${literatur.length} unklar=${uneindeutig.length}) | ${Date.now() - t0}ms`)
